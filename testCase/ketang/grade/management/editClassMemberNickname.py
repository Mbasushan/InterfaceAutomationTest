...
class EditClassMemberNickname(unittest.TestCase):
    ...
    def test_editClassMemberNickname(self):
        ...
        params={'access_token':access_token,'class_id':'1000','user_id':'20061','nickname':'接口测试修改昵称'}
    ...
    def test_editClassMemberNickname_noToken(self):
        """管理员修改成员昵称---该用户不是管理员"""
        access_token = Token.getToken()
        params={'access_token':access_token,'class_id':'1001','user_id':'20061','nickname':'接口测试修改昵称'}
        response=requests.post(self.base_url,params)
        result=response.json()
        print(result)
    ...
    def test_editClassMemberNickname_noClassId(self):
        """管理员修改成员昵称-未传班级id"""
        access_token = Token.getToken()
        params={'access_token':access_token,'class_id':'','user_id':'20061','nickname':'接口测试修改昵称'}
        response=requests.post(self.base_url,params)
        result=response.json()
        print(result)
    ...
    def test_editClassMemberNickname_noUserId(self):
        """管理员修改成员昵称-未传user_id"""
        access_token = Token.getToken()
        params = {'access_token': access_token, 'class_id': '1000', 'user_id': '', 'nickname': '接口测试修改昵称'}
        response = requests.post(self.base_url, params)
        result = response.json()
        print(result)
        self.assertEqual(result['error'], '参数错误')
    ...
    def test_editClassMemberNickname_noNickname(self):
        ...
        access_token = Token.getToken()
        params = {'access_token': access_token, 'class_id': '1000', 'user_id': '20061', 'nickname': ''}
        response = requests.post(self.base_url, params)
        result = response.json()
        print(result)
        self.assertEqual(result['error'], '参数错误')